...
/**
 * 歩行中の body 全体上下(bobbing)
 *
 * - `bodyBobbing` が有効な間、脚の実際の動きから体の高さを計算する。パラメータで\
 *   周期・位相を独立に持つ(=脚と別々に回る sin 波を近似で合わせる)のではなく、\
 *   脚の ref の現在値を毎フレーム読むため常に正確に連動する
 * - 'bob': 左右脚の `position.y` オフセットのうち最も低い方(最も伸びている方)を\
 *   体の持ち上げ量に変換する
 * - 'swing': 左右脚の `rotation.x` の余弦(垂直に近いほど大きい)のうち大きい方を\
 *   「支持度」として体の高さに変換する
 * - 'none' または `bodyBobbing` が無効な間は 0 に固定する。脚自体が\
...
export const useBodyBobbingAction = (
  props: Pick<BoxBotModelProps, 'bodyBobbing' | 'legMotion'>,
  legY: number,
): void => {
  const { bodyBobbing, legMotion } = props

  const { leftLegRef, rightLegRef, walkingBobRef } = useBoxBotRefs()

  useFrame(() => {
    if (!walkingBobRef.current || !leftLegRef.current || !rightLegRef.current)
      return

    if (!bodyBobbing || legMotion === 'none' || legMotion == null) {
