...
export interface BoxBot3DProps
   ...
   *
   * - 省略時は設置領域 * `DISPLAY_RATIO`(正方形)。bot が設置領域からわずかにはみ出す
   * - 明示すると更に縦へ広げる。逸脱率(overscan)ぶん fov を広げるため、逸脱ぶんは
   *   余白になり bot の見かけの大きさ・位置は不変に保たれる
   ...
   *
   * - 省略時は設置領域 * `DISPLAY_RATIO`(正方形)
   * - 明示すると更に横へ広げる。高さ・fov は変えないため bot の見かけの大きさは不変で、
   *   左右に見える範囲(余白)だけ増える
   ...
   * カメラの視野角(度)
   *
   * - 省略時は基準画角を overscan(表示領域 / 設置領域)ぶん広げた値。overscan=1 なら
   *   基準画角固定で、`style.height` を変えると bot もそれに比例して拡大縮小する
