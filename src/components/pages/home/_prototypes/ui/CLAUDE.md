...
- 各バリエーション、単独で確認できる `index.stories.tsx` を持つ(`proto-02` は削除予定、移行の土台として当面残置)
...
- `_components/`: 複数バリエーション共通の実装置き場
  - `bot-overlay`: bot 表示領域と同サイズのオーバーレイコンテナ(circle/square/anchor 共通の土台。実装済、`action-anchor` のみ移行済)
- `_story-decorators`: 各 `index.stories.tsx` 共通の Storybook decorator(`withBot`: bot あり、`withoutBot`: bot なし。実装済)。`_components/bot-overlay` を参照するため `_components/` と同列に配置(同列 import 回避)
...
現在の呼び出し元は `proto-02/index.tsx`(削除予定、変更しない)。`actionLayout` prop(Storybook: `Default`/`Single`/`Circle`/`Square`/`Anchor`/`Ring`/`GroundRing`)で切替表示する。`action-anchor` は単独 `index.stories.tsx` で `_components/bot-overlay` 経由の確認が可能(`proto-02` 側は旧来の実装のまま、未統合)。
