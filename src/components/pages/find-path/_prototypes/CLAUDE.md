...
- `proto-01`: 試作。`prototypes/stage/stage-06` (遠近ステージ + actor 位置 ref 版) をページ枠へマウントした土台。ゲーム内容は経路プランニング制 (`planned-path` へ積み上げ → まとめて「実行」で tick 進行)
  ...
  - `_stores/items`: proto-01 固有の汎用アイテム store（issue #181）。energy store とは責務を分け、回復アイテム/回復スポットの状態（未回収フラグ・スポット残量）を保持する。`ItemInstance.stock` の有無で「回復アイテム」（未指定、1個ずつ使い切り）と「回復スポット」（指定、指定回数で枯渇しうる）を区別する。`kind`（`ItemKind`）は将来のアイテム種類拡張用（issue #181 時点では `energy-recovery` のみ）。`ItemInstance.cell` は `ActorNodeRegistry` の `GridPosition` 型をそのまま使う。障害物（`_lib/obstacle.ts`、静的定数）とは責務を分けたまま独立管理し、`_lib/get-cell-contents.ts` の `getCellContents(cell, itemState)`（pure function）がアイテム store と障害物定数を横断的に問い合わせる（issue #137、PR #196 レビュー対応）。hover 説明文言は `_lib/describe-cell-content.ts` の `describeCellContent` で解決、アイテムの文言・絵文字・className は `_lib/item-presentation.ts` の `getItemPresentation`（`ItemKind` ベースの辞書）に一元化。新種類追加時はこの辞書へエントリを足すだけで対応可
  - `constants.ts` の `RECOVERY_ITEM_CELLS`/`RECOVERY_SPOT_CELLS`: 回復アイテム/回復スポットの初期配置一覧（座標・回復量・stock は仮値、design.md 懸念・リスク）
  - `_components/item-layer/`: `ItemStore` を購読し、未消費（消費済みは store から削除済み）の全アイテムを種類問わず表示する非対話レイヤー（`ObstacleLayer` 同型）。表示（絵文字・className）は `getItemPresentation` で解決するため、種類が増えてもレイヤー自体を増やす必要はない（旧 `RecoveryItemLayer`/`RecoverySpotLayer` を統合、issue #137）。回復処理自体は `use-find-path-tick` の `applyNextStep`（`moveActor` 実行後・`energy.consume` 実行前）が行う
...
- `proto-03`: 試作。`prototypes/stage/stage-07`（hex グリッド版、issue #162）をページ枠へマウントした版。移動方式は proto-02 と同じ隣接クリック逐次移動だが、`Stage07` の `useHexMove` に内蔵済みのためページ側は `onCellChange` を受けるだけ
  ...
  - `_components/move-target-layer/`: 移動可能マス（現在地の隣接6方向）の表示演出レイヤー。現在地セル変更（進入開始）で非表示にし、bot がそのセルの中心に到達（`Stage07-cell-reach`、`_hooks/use-reached-cell.ts`）してから 80ms後に演出開始する。演出の段階は `useMoveTargetLayer` が管理する。表示完了後は次の進入開始まで維持する。演出は `mode` prop（`MoveTargetDisplayMode`）で切替可能: `scatter`（bot マスへ集合 → 対象マスへ散開、既定）/ `instant`（transitionなしで対象マスへ即座に出現）/ `fade`（対象マスの位置で opacity 0→1、位置移動なし）
  ...
  - `_lib/get-cell-contents.ts`・`_lib/item-presentation.ts`・`_lib/describe-cell-content.ts`・`_components/item-layer/`: proto-01 の同型を axial 座標へ移植（`ObstacleLayer` 同型の非対話レイヤー）。title 表示は `CellTitleProvider`（`getCellContents`/`describeCellContent` から合成）へ配線
  - `_events/`: proto-03 専用の find-path EventTarget（issue #226）。proto-01 の `_events` と同構成だが EventMap は別（混在回避）。全イベントを cancelable で発行し、dispatcher は listener に拒否（`preventDefault()`）されると `false` を返す。UI は担当範囲の情報（経路の提示・実行）のみ発行し、EN 等のゲーム要素による実行可否は listener 側で判定する（`docs/concept/implementation/ui-jurisdiction`）
