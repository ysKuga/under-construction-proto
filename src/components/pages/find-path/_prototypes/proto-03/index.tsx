...
const FindPathProto03Content = (props: FindPathProto03ContentProps) => {
  ...
  const [followingPath, setFollowingPath] = useState<HexCell[]>([])
  ...
  const isAutoMoving = followingPath.length > 0
   ...
   * - 経路は `followingPath` へ固定し、自動移動中はその残りをプレビューする
  ...
  const handleCellChange = (cell: HexCell) => {
    ...
    setWaypointFlowState('idle')
    setWaypoints([])
    // 自動移動の 1 マス目は「実行」と同じタスク内で呼ばれ、この関数が古い
    // クロージャのままになるため、自動移動中かを見ずに関数型更新で数える
    // （自動移動中でなければ `followingPath` が空のため表示に影響しない）
    setFollowedCount((count) => count + 1)
    markVisited(cell)
      ...
      >
        ...
        >
          ...
          <PathPreviewLayer
            cols={GRID.cols}
            hexSize={HEX_SIZE}
            passedCount={isAutoMoving ? followedCount : 0}
            path={isAutoMoving ? followingPath : previewPath}
            rows={GRID.rows}
          />
