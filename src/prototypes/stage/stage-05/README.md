...
- tilt を強くしても bot がマスから外れる問題は解決済み（主因は Canvas < 設置領域のずれ、`resize={{ offsetSize: true }}` で解消）。四隅の外縁 bot で tilt 全域を確認。
...
- 奥行きセルでの actor 設置位置の微妙なズレ（`translate` の Y は中間セル基準で調整済み、最奥行は浮き気味）。許容範囲。
