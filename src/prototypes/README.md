...
## 実装ルール検討 (prototypes/CLAUDE.md 設置検討)

prototypes の実装バージョンを上げる際 (stage-01 → 02 → 03 のように) コードをコピーしていると際限なくコードが増える。\
現在実装より若いバージョンからの import を許容する方針を検討する。

- コード増加の抑制。
- 変更箇所がわかりやすくなる (差分がバージョン間の import 関係で追える)。

## コード分割方針の検討

types, components, hooks など、ファイル種別ごとの分割 (`sample-01.components.tsx`, `sample-01.types.ts` など) を全体的な実装方針として進めていきたい。
