...
- [x] `_hooks/_utils/` を廃止し、汎用ラッパー (`useTimeControl03EventDispatcher`/`useTimeControl03EventListener`) を `_hooks/` 直下へ移動
- [x] 個別 listener 実装 (`use-reset-all-listener` → `use-reset-all-event-listener`) を `_hooks` と同列の `_event-listener/` へ移動、`useResetAllListener` → `useResetAllEventListener` にリネーム
- [x] `useResetAll` (action-bar 側の薄いラッパー hook) を廃止し、`useActionBar` (`index.hooks.ts`) で `useTimeControl03EventDispatcher` を直接呼ぶ形に変更
...
- **`_event-listener` は `_hooks` と同列 (`_events` 直下) に配置**。`_stores` の各 store ディレクトリと同格で、個別イベント実装を並べる場所として独立させた。`_event-listener` 配下の実装は `_hooks` 直下 (汎用ラッパー) に依存してよいと明文化した。
- **`useResetAll` は hooks 構成分割規約 (`.claude/rules/react/hooks.md`) の例外として廃止**。同規約は `useActionBar` が `_hooks/` 配下の部品 hook を組合せるだけの構成を求めており、`useDispatchDecision`/`useProgressMode` との一貫性は崩れる。dispatcher オブジェクト化により `useResetAll` が「1行のオブジェクトアクセスをラップするだけ」になり、部品 hook として切り出す価値が薄れたことを優先した判断。

## 懸念・リスク

- grep 衝突回避 (イベント名の一意な識別子化) は未解決のまま。他ドメインへの event 基盤展開時に再検討が必要になる可能性がある。
- listener 側 (`useTimeControl03EventListener` / `useResetAllEventListener`) は今回未変更。dispatcher 側とインターフェースの対称性が崩れている (dispatch はオブジェクトアクセス、listen は引数渡しのまま)。
- `useResetAll` 廃止で hooks 構成分割規約からの例外が生まれた。今後同種の薄いラッパー hook (dispatch 系) が増えた場合、規約側に例外規定を追記するか個別判断を続けるか要検討。
