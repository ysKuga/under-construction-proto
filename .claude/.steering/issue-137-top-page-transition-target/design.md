...
- （将来検討）旋回（曲がり角）前後の減速等、連続的な速度プロファイル制御。CSS transition + DOM 直書きの現行方式では区間ごとの一定速度までが限界、実現するには Web Animations API への移行が要る（decision-records.md 2026-09-16）
- マス上オブジェクト（障害物・回復アイテム・回復スポット等）の説明表示が未整備。現状は絵文字表示のみでホバーしても内容不明。className prefix 付与の汎用方針は `docs/concept/ideas/README.md`「ユーザビリティ・用語説明」参照、find-path 側はまず `title` 属性による暫定表示を検討
- （将来検討）マスホバー/選択時、そのマスに存在する要素（障害物・アイテム等）の一覧表示。今回の対応（`title` 属性表示）とは別スコープでもよい
