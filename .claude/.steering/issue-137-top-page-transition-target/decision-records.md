...
- 2026-09-19: PR #196 レビュー対応。(1) `Stage07`/`GeoLayer` の `cellTitle` prop（props drilling）を撤去し、`stage-07/_contexts/cell-title` の `CellTitleContext` へ変更。stage-07 は find-path 固有概念を持たないため型のみ定義し、実体（`getCellTitle` の中身）は呼び出し元（proto-03: `CellTitleProvider`）が注入する。GeoLayer は `useCellTitle()` をループ外で1度だけ呼び出し、戻り値の関数をセルのループ内で使う（hook をループ内で呼べないため）。(2) proto-01 の `PlannedPathLayer` が独自に持っていた「isObstacleCell + アイテム検索」の分岐ロジックを廃止。`ItemStore`（`_stores/items`）へ障害物（`obstacleCells`）を統合し、`getContentsAtCell(cell)` でセル上の要素（障害物・アイテム）を種類問わず横断的に取得できるようにした。hover 説明文言は `_lib/describe-cell-content.ts` の `describeCellContent`（`ItemKind` ベースの辞書）で解決、新しい種類のアイテムが増えても辞書へエントリを足すだけで対応できる
