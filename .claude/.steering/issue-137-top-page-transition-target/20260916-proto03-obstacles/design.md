...
- hex 座標（`HexCell`, axial `q`/`r`）での障害物データ定義方法未検討
- 選択拒否・視覚化を `GeoLayer`/`MoveTargetLayer` のどちらに持たせるか未検討
- `useHexMove` の隣接移動判定への組込み方法未検討（tick 駆動と異なり、1 手ごとに即時 `moveActor` するため「次マス障害物ならスキップ」でなく「隣接候補から除外」に近い形になる可能性）
