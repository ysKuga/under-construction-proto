...
- [ ] 経路上に中継点を設定できるようにする
  - 目的地クリック時、思考の吹き出し(発言吹き出しと区別)を表示、クリックで中継点選択モードへ移行
    - モード切替・設置/除去(state 切替)・bot 頭上への位置調整(`createPortal` + actors store 経由のオーバーレイコンテナ)まで実装済み(decision-records.md 2026-09-23)
    - 表示調整は次段階、別セッションで対応
      - モード未選択時(思考吹き出し): bot と吹き出しの間に小さい丸を 2-3 個並べ、CSS でふわふわさせる
      - モード選択時(発言吹き出しへ切替、文言「中継点！」): bot に向いた三角形（吹き出しの「しっぽ」）を追加
